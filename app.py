...
def process_pdf(pdf_files):
    ...
    try:
        ...
        if not extracted_text and not extracted_tables and not extracted_images:
            return None, None, None, "No content extracted from the PDFs. Please upload valid PDFs."
...
def upload_pdf():
    try:
        ...
        session['extracted_tables'] = extracted_tables
        session['extracted_images'] = extracted_images
        print("PDFs uploaded and content extracted successfully.")
...
# API route to ask questions based on the uploaded PDFs
@app.route('/ask', methods=['POST'])
def ask_question():
    try:
        if 'extracted_text' not in session:
